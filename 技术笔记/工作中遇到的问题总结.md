...
32. 整套流程梳理：
    ...
    3.  客户端根据统跳打开小程序的wx-article页面，该页面拿到origin去请求midas接口拿到对应文章的url，并在该页面显示
    4.  识别页面中的二维码跳转到对应的公众号，然后当用户点击关注之后，微信会给公众号后台配置的接口地址发送消息，然后由服务端处理关注事件拿到对应的信息（取关也是相同操作）
    ...
    6.  通过链接进入我们的h5页面
        ...
        3.  这个bug
            ...
            ![微信跳转bug](../img/微信跳转bug.png)

            是因为没有在公众号后台（不是小程序后台）配置网络授权域名
    7.  h5页面打开客户端形成闭环
33. [webpack插件DefinePlugin](https://juejin.cn/post/6844903458974203911):这个插件用来定义全局变量，在webpack打包的时候会对这些变量做替换。
